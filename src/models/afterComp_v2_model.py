"""CPC after compression V2 - adds extra encoder before temporal module in decoder."""
...
class AfterCompV2Encoder(nn.Module):
    """V2 Encoder - same as V1, compress first."""
    
    def __init__(self, compressed_size: int = 32, num_bits: int = 2):
        super(AfterCompV2Encoder, self).__init__()
...
class AfterCompV2Decoder(nn.Module):
    """V2 Decoder - reconstruct, then extra encoder, then CPC."""
    ...
    def __init__(self, compressed_size: int = 32, hidden_size: int = 64, num_bits: int = 2,
                 ...
                 time_window: int = 10):
        super(AfterCompV2Decoder, self).__init__()
        ...
        # Extra encoder before predictive coding (V2 difference)
        self.BeforePred_Encoder = nn.Sequential(
            QuantizedConv2d(2, 16, kernel_size=3, padding=1, num_bits=num_bits),
            nn.ReLU(),
            ResNetBlock(16, num_bits),
            ResNetBlock(16, num_bits),
            QuantizedConv2d(16, 16, kernel_size=1, num_bits=num_bits)
        )

        # Predictive Coding - temporal module on encoded reconstructed signal
        temporal_input_size = 16 * 13 * 32  # Different from V1 (was 2 * 13 * 32)
    ...
    def forward(self, latent_message, batch_size, subband_num, port_num, channel_num):
        ...
        # Extra encoder before predictive coding (V2 difference)
        beforePredictive = self.BeforePred_Encoder(reconstructed)
        
        # Handle temporal windowing for CPC
        batch_size, c, h, w = beforePredictive.shape
        ...
        x_temporal = beforePredictive[:complete_samples].view(-1, self.time_window, c, h, w)
...
class AfterCompV2Autoencoder(nn.Module):
    """CPC after compression V2 autoencoder."""
    
    def __init__(self, compressed_size: int = 32, hidden_size: int = 64, num_bits: int = 2,
                 temporal_type: str = 'gru', num_layers: int = 1, dropout: float = 0.0,
                 num_heads: int = 4, feedforward_dim: int = 256, pred_steps: int = 5,
                 time_window: int = 10):
        super(AfterCompV2Autoencoder, self).__init__()
        
        self.encoder = AfterCompV2Encoder(
            compressed_size=compressed_size,
            num_bits=num_bits
        )
        
        self.decoder = AfterCompV2Decoder(
            compressed_size=compressed_size,
            hidden_size=hidden_size,
            num_bits=num_bits,
